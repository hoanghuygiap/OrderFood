    ...
    <AdminLayout>
      ...
      <div className="container mt-4">
        <h3 className="text-center text-primary mb-4">
          <i className="fas fa-list-alt me-1"></i>SearchOrder
        </h3>
        <form onSubmit={handleSearch} className="d-flex mt-3" style={{maxWidth: '600px', margin: '0 auto'}}>
            <input
              type="text" onChange={(e) =>setSearchTerm(e.target.value)} placeholder="Enter order number" className="form-control"
          ...
          <tbody>
            ...
            orders.map((order, index) => (
              <tr key={order.id}>
                ...
                <td>
                  <Link
                    to={`/admin-view-order-detail/${order.order_number}`}
